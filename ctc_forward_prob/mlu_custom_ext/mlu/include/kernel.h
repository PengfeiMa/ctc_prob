...
__mlu_func__ void __bang_exp(float *dst, float *src, int elem_count) {
  // log2e = 1.4426950408889634
  int tmp = 0x3fb8aa3b;
  float log2e = *(float *)&tmp;
  __bang_mul_scalar(dst, src, (float)log2e, elem_count);
  __sync_compute();
  __asm__ volatile(
      "pow2.async.nram.f32 "
      "[%[dst]], [%[src]], %[size];\n\t" ::[dst] "r"(dst),
      [src] "r"(dst), [size] "r"(elem_count));
  __sync_compute();
}
#endif
